...
{ 
	...
	int charPrinter = 2*i - 1;
	for (int l = 0; l < charPrinter; ++l)
	{
	    printf("%c", *k);
	} 
    ...
    for (int i = heightAndWidth / 2; i >= 0; --i)	// Repeat the first half but mirror it
    {
        for (int j = 0; j < num - i; ++j)         // Print the spaces so that the edge of the diamond follows the proper structure****
        {
            printf("%c", ' ');
        ...
        for (int j = num - i; j >= 0; --j)         // Print the spaces in the same fashion as the first part but mirrored to create symmetry
        {
            printf("%c", ' ');
        }
